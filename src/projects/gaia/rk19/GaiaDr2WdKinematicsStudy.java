...
import projects.gaia.lrh18.util.GaiaSourceUtil;
import projects.gaia.util.RenormalisedUnitWeightError;
 ...
 * 
 * TODO: outlier rejection in least squares solution?
 *
 * @author nrowell
 * @version $Id$
 */
public class GaiaDr2WdKinematicsStudy {

	/**
	 * The logger.
	 */
	private static final Logger logger = Logger.getLogger(GaiaDr2WdKinematicsStudy.class.getName());
	
	/**
	 * Location of input data file: the exact CSV file as downloaded from the Gaia archive.
	 * This is assumed to have applied WD selection and thresholds on parallax significance and other quality criteria.
	 */
//	private static final File data = new File("/home/nrowell/Projects/Astronomy/gaia_WDs/Gaia_DR2_WDLF/queries/wd_sample/Gaia_WDs_selection-result.csv");
	private static final File data = new File("/home/nrowell/Projects/Astronomy/gaia_WDs/Gaia_DR2_WDLF/queries/wd_sample/All_stars_within_250pc-result.csv");
//	private static final File data = new File("/home/nrowell/Projects/Astronomy/gaia_WDs/Gaia_DR2_WDLF/queries/wd_sample/tmp.txt");
	...
	/**
	 * Threshold on renormalised unit weight error.
	 */
	static double ruwe_max = 1.40;
...
//	static double vtan_max = 1000.0;
//	static double vtan_min = 200.0;
	
	/**
	 * Absolute G magnitude binning parameters
	 */
	static double absG_min = 10.0;
	static double absG_max = 15.5;
	
	/**
	 * Magnitude bin step size for CONSTANT_MAG_STEP
	 */
	static double absG_step = 0.125;
//	static double absG_step = 0.5;
	 ...
	 */
	static enum BIN_SCHEME {CONSTANT_MAG_STEP, CONSTANT_STARS};
	
	/**
	 * The chosen type of binning.
	 */
//	static BIN_SCHEME binScheme = BIN_SCHEME.CONSTANT_STARS;
	static BIN_SCHEME binScheme = BIN_SCHEME.CONSTANT_MAG_STEP;
	...
	/**
	 * Lower limit on absolute G magnitude to use in the determination of LSR motion etc. This marks the point brighter than
	 * which there is no correlation between the magnitude and the average age of White Dwarfs.
	 */
	static double absG_ageCorrelation_min = 10.0;
	
	/**
	 * Upper limit on absolute G magnitude to use in the determination of LSR motion etc. Points fainter than this are judged
	 * to be too noisy to include.
	 */
	static double absG_ageCorrelation_max = 15.0;
	...
	public static void main(String[] args) throws IOException {
		...
		// Bin according to absolute G magnitude
		...
		for(int bin=0; bin<magBinnedWds.size(); bin++) {
			...
			ImageIO.write(plot, "png", new File("/home/nrowell/Publications/Preparation/Gaia_WDs_kinematics/figures/cdf/"+String.format("%.5f.png", mag)));
		}
		...
		for(int i=0; i<magBinnedWds.size(); i++) {
			...
			if(mag < absG_ageCorrelation_min || mag > absG_ageCorrelation_max) {
				continue;
			}
	...
	private static void plotVelocityDispersion(Range[] magBins, Matrix[][] velocityDispTensor) {
		...
		script.append("set xrange ["+absG_min+":"+absG_max+"]").append(OSChecker.newline);
...
//		script.append("set xzeroaxis lt 1 lw 1 dt 2 lc rgbcolor 'black'").append(OSChecker.newline);
	...
	private static void plotVelocityEllipsoidProjections(Range[] magBins, Matrix[][] velocityDispTensor) {
		...
		for(int k=0; k<3; k++) {
			...
			script.append("set cbrange ["+absG_min+":"+absG_max+"]").append(OSChecker.newline);
	...
	private static Collection<ExtendedGaiaSource> applyInitialSelections(Collection<ExtendedGaiaSource> allStars) {
		...
		for(ExtendedGaiaSource star : allStars) {
			...
			// Unit weight error
			double uwe = Math.sqrt(star.astrometric_chi2_al / (star.astrometric_n_good_obs_al - 5));
			
			// Selection on RUWE from GAIA-C3-TN-LU-LL-124 & 'Gaia DR2 Astrometry' talk @ IAU 30:
			double ruwe = Double.NaN;
			
			if(star.phot_g_mean_mag > 3.595 && star.phot_g_mean_mag < 21.005) {
				double u0 = RenormalisedUnitWeightError.getU0(star.phot_g_mean_mag, bpRp);
				ruwe = uwe / u0;
			}

			if(ruwe > ruwe_max || Double.isNaN(ruwe)) {
				reject = true;
				count[0]++;
			}
	...
	private static RangeMap<ExtendedGaiaSource> binByAbsG(Collection<ExtendedGaiaSource> wds) {
		...
		if(binScheme == BIN_SCHEME.CONSTANT_MAG_STEP) {
			// Map the stars by magnitude
			magBinnedWds = new RangeMap<>(absG_min, absG_max, absG_step);
		}
			...
			for(ExtendedGaiaSource star : wds) {
				...
				if(gMag >= absG_min && gMag <= absG_max) {
					gMags.add(gMag);
				}
			}
