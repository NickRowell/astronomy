 ...
 * @author nickrowell
 */
public class HighMuSurvey extends Survey{
    ...
    double lowMuLimit = 0.18;
    double highMuLimit = 1.000;     // New limit 13/08/2010

    //+++ Max acceptable difference between first and second epoch R magnitudes +++//
    public double delRLim = 0.25;

    public HighMuSurvey(String FOOTPRINT) throws Exception{
        ...
        for(int f = 1; f<includeFields.length; f++){
            for(int h=0; h<2; h++){
                ...
                if(RejectField.contamination(f, ((h==0) ? "N":"S")) &&
                   ...
                   RejectField.epochSpread(f, ((h==0) ? "N":"S")))
	...
	for(int h = 0; h<2; h++){
		...
		if(this.includeFields[FIELD][h]){
			...
			if(d_upper>dmax){ d_upper = dmax;}
                        ...
                        double vt_min = 4.74 * d_midStep * this.lowMuLimit;
